...
jax_t, jax_q, jax_pi = solver.integrate(jnp.array(q0), jnp.array(pi0), t0, t_sample_count)
...
axs[0].plot(t, jax_q, label='JAX', linestyle='dashed')
axs[0].legend()
axs[0].set_title('q')

# Plot your second chart on the second subplot
axs[1].plot(t, original_results[1], label='Original')
axs[1].plot(t, jax_pi, label='JAX', linestyle='dashed')
axs[1].legend()
axs[1].set_title('pi')
